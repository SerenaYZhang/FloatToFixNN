...
func generateValidation() {
	t1 := time.Now()
	testFile, _ := os.Open("Age_Detection/Age_Detection_Train_File/data/Test_resized.csv")
	file, _ := os.Create("Age_Detection/Age_Detection_Train_File/data//validation.csv")
	defer file.Close()
...
func mnistTrain(net *Network) {
	...
	for epochs := 0; epochs < 1; epochs++ {
		testFile, _ := os.Open("Age_Detection/Age_Detection_Train_File/data/Train_resized.csv")
		r := csv.NewReader(bufio.NewReader(testFile))
		for {
			record, err := r.Read()
			if err == io.EOF {
...
func mnistTrainForPlot(net *Network) {
	...
	for epochs := 1; epochs <= 5; epochs++ {
		...
		for {
			...
			net.Train(inputs, targets)
			if(count % 1000 == 0){
				checkFile, _ := os.Open("Age_Detection/Age_Detection_Train_File/data/validation.csv")
...
func mnistPredict(net *Network) {
	t1 := time.Now()
	checkFile, _ := os.Open("Age_Detection/Age_Detection_Train_File/data/Test_resized.csv")
	defer checkFile.Close()
